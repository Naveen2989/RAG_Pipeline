...
if os.getenv("LANGSMITH_TRACING", "false").lower() == "true":
    client = Client()
    project_name = os.getenv("LANGSMITH_PROJECT", "travelagentOrchestration")
...
config_list = [
    {
        "model": "llama3.1:latest",  # Ollama local model
    ...
    def initiate_conversation(self, message: str):
        ...
        if client:
            run = client.create_run(
                ...
                name="TravelAgentConversation",
                inputs={"message": message},
                run_type="tool"  # or "chain", "llm", etc. as appropriate
            )

        # Run conversation
        response = self.user_proxy.initiate_chat(self.travel_assistant, message=message)
...
if __name__ == "__main__":
    ...
    result = travel_system.initiate_conversation(
